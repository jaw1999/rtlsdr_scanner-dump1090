...
from src.signal_processing.spectrum_widget import SpectrumWidget
from src.signal_processing.waterfall_widget import WaterfallWidget
import logging
    ...
    def start_scan(self):
        try:
            if self.sdr_controller.is_active:
                ...
                logging.warning("SDR is active. Cannot start scan.")
                return
            ...
            if not (1e6 <= center_freq <= 1.7e9):  # Updated to 1.7 GHz
            ...
            # Handle gain
            if gain.lower() == 'auto':
                self.sdr_controller.gain = None  # Set to None for auto gain
            else:
                try:
                    self.sdr_controller.gain = float(gain)
                except ValueError:
                    raise ValueError("Gain must be 'auto' or a numeric value.")
